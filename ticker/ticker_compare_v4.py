...
from tickers import raw_weights, tickers
...
for current_date, day_5m in grouped_5m:
    ...
    # ----------------------------------------------------
    # 2. TOP GRAPH: Intraday % Change & Weighted Return
    # ----------------------------------------------------
    ...
    # --- Compute Weighted Portfolio Series Intraday ---
    portfolio_5m = sum(
    ...
    # --- Top Plot: Bold Black Portfolio Line ---
    ax_top.plot(
        pct_5m.index,
        portfolio_5m,
        color="black",
...
mult_13 = (
    math.exp(sum(math.log(x / 100 + 1) for x in data_13) / n) - 1
) * 100
mult_close = (
    math.exp(sum(math.log(x / 100 + 1) for x in data_close) / n) - 1
) * 100
monthly_return = (
    math.exp(
        sum(math.log((y - x) / 100 + 1) for x, y in zip(data_13, data_close))
        / period
    )
    - 1
) * 100
...
with open(os.path.join(output_dir, "summary.txt"), "w") as f:
    for i, j in zip(data_13, data_close):
        f.write(
            f"13:00: {i:+.2f}% | Close/Latest: {j:+.2f}% | Diff:"
            f" {(j-i):+.2f}%\n"
        )
    f.write("\n")
    f.write(
        f"13:00: {mult_13:+.2f}% | Close/Latest: {mult_close:+.2f}% | Diff:"
